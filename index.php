...
try {
    ...
    $stmt->execute();
    $cupoLleno = ((int)$stmt->fetchColumn() >= $cupoGlobal);
...
.form-check-input:focus {
  border-color: #f8c200;
  box-shadow: 0 0 0 0.2rem rgba(248,194,0,0.25);
}
...
<!-- Mensaje de cupo global lleno -->
<div id="mensajeCupoLleno" class="text-center mb-4" style="display:<?= $cupoLleno ? 'block' : 'none' ?>;">
  <h2 class="fw-bold mb-3" style="color:#f8c200;">🎵 Listado completo</h2>
  <p style="color:#ccc;">El cupo de canciones está lleno en este momento.<br>Espera a que suenen algunas canciones e intenta nuevamente.</p>
</div>
...
<div id="contenedorFormulario" class="form-container mx-auto mb-4" style="max-width:600px;">
  ...
  <form id="formSolicitud">
    ...
    <div class="input-group-custom">
      ...
      <select name="genero" id="selectGenero" required <?= !$puedeEnviar ? 'disabled' : '' ?>>
    ...
    <div class="form-check mt-3 text-center">
      <input class="form-check-input" type="checkbox" id="checkTerminos" required <?= !$puedeEnviar ? 'disabled' : '' ?>>
      <label class="form-check-label" for="checkTerminos" style="font-size: 0.85rem; color: #ccc; cursor: pointer;">
        Al enviar mi solicitud declaro que he leído los 
        <a href="#" id="linkTerminos" class="text-decoration-none" style="color: #f8c200; font-weight: 600;">
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content bg-dark text-light">
      ...
      <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
        ...
        <ol class="ps-3">
          <!-- ITEM 1: Límite de canciones -->
          <li class="mb-2">
            <strong>Máximo <?php echo $numeroLimite; ?> <?= $numeroLimite == 1 ? 'canción' : 'canciones' ?> por persona.</strong><br>
            Puedes realizar hasta <strong><?php echo $numeroLimite; ?> <?= $numeroLimite == 1 ? 'solicitud' : 'solicitudes' ?> en total</strong>.  
            Así todos tendrán la oportunidad de hacer sonar su tema favorito, cuando <?= $numeroLimite == 1 ? 'tu canción suene' : 'tus canciones suenen' ?> y el DJ habilite más solicitudes tendrás la oportunidad de pedir más.
          </li>

          <!-- ITEM 2: Una canción por formulario -->
          <li class="mb-2">
          ...
          <!-- ITEM 3: NUEVO - Selección correcta de género -->
          <li class="mb-2">
            <strong>⚠️ Selecciona el género correcto de tu canción.</strong><br>
            Es <strong>muy importante</strong> que elijas el género musical correcto en el formulario. 
            Si seleccionas un género equivocado (por ejemplo, marcas "Salsa" para una canción de Vallenato), 
            <strong>tu solicitud será automáticamente rechazada</strong> por el sistema. 
            <br><br>
            <span class="text-warning">
              <i class="fas fa-exclamation-triangle me-1"></i>
              Si un género no aparece en el listado, significa que:
            </span>
            <ul class="mt-2 small">
              <li>No hay cupo disponible para ese género en este momento</li>
              <li>Hay muchas canciones de ese género en espera</li>
              <li>El DJ lo ha deshabilitado temporalmente</li>
            </ul>
            Por favor, espera unos minutos y vuelve a intentarlo, o elige otro género disponible.
          </li>

          <!-- ITEM 4: Canciones conocidas -->
          <li class="mb-2">
...
(function() {
  ...
  const CONFIG = {
    ...
    cupoLleno: <?= $cupoLleno ? 'true' : 'false' ?>
  };
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar géneros activos dinámicamente
    cargarGenerosActivos();
    
    // 🔥 ACTUALIZACIÓN EN TIEMPO REAL: Recargar géneros cada 5 segundos
    setInterval(cargarGenerosActivos, 5000);
    
    console.log('✅ Todos los módulos inicializados correctamente');
    console.log('🔄 Actualización automática de géneros: cada 5 segundos');
  });
  ...
  // ============================================
  // ENLACE PARA REABRIR MODAL DE TÉRMINOS
  // ============================================
  function initEnlaceTerminos() {
    $(document).on('click', '#linkTerminos', function(e) {
      e.preventDefault();
      const modalEl = document.getElementById('modalTerminos');
      if (modalEl) {
        const modal = new bootstrap.Modal(modalEl);
    ...
    console.log('✅ Enlace de términos inicializado');
  }
  ...
  function initFormulario() {
    ...
    $(document).off('submit', '#formSolicitud').on('submit', '#formSolicitud', async function(e) {
      ...
      try {
        ...
        } else if (result.status === 'cupo_lleno') {
          Swal.fire({
            icon: 'info',
  ...
  function bloquearFormulario() {
    $('#formSolicitud input, #formSolicitud select, #btnEnviar, #checkTerminos').prop('disabled', true);
    $('#btnEnviar').text('Límite alcanzado').addClass('disabled');
  }

  function desbloquearFormulario() {
    $('#formSolicitud input, #formSolicitud select, #btnEnviar, #checkTerminos').prop('disabled', false);
  ...
  // CARGAR GÉNEROS ACTIVOS DINÁMICAMENTE
  // ============================================
  function cargarGenerosActivos() {
    fetch('controller/generos_activos.php')
      .then(res => res.json())
      .then(data => {
        if (data.status === 'success') {
          ...
          data.generos_activos.forEach(genero => {
          ...
          // Restaurar selección si el género sigue disponible
          if (valorActual && data.generos_activos.includes(valorActual)) {
            select.value = valorActual;
          }
          
          console.log('✅ Géneros cargados:', data.generos_activos);
          
          // Si todos los géneros están bloqueados
          if (data.generos_activos.length === 0) {
            select.innerHTML = '<option value="">No hay géneros disponibles</option>';
            select.disabled = true;
            
            // Mostrar alerta si estaba habilitado antes
            if (!select.disabled) {
              Swal.fire({
                icon: 'warning',
                title: 'Géneros no disponibles',
                text: 'Todos los géneros han sido bloqueados temporalmente',
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
                timer: 3000
  ...
  function initControlFormulario() {
    ...
    const mensajeCupoLleno = $('#mensajeCupoLleno');
    const relojRestante = $('#relojRestante');

    if (CONFIG.cupoLleno) {
      mostrarCupoLleno();
    } else if (!CONFIG.formularioActivo) {
