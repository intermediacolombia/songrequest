...
<div class="container-fluid">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="fw-bold mb-0">Panel de Solicitudes</h1>
	  ...
	  <!-- Campo para editar el número máximo de solicitudes -->
...
<div class="row row-cols-1 row-cols-md-4 g-4 text-center mb-4" id="contadores">
    ...
    <h5 class="fw-bold">Auto detectar canciones reproducidas</h5>
    <label class="form-label mt-2">Selecciona el archivo M3U del historial de VirtualDJ:</label>
    <input type="file" id="m3uInput" accept=".m3u,.txt" class="form-control">

    <p class="text-muted small mt-2">
        El archivo no se sube al servidor, solo se analiza localmente cada 5 segundos.
    </p>

    <div id="autoStatus" class="mt-2 fw-bold text-success"></div>
</div>
...
let contenidoM3U = "";
let intervalo = null;
let ultimaCancionProcesada = ""; // <<< evita el bucle

document.getElementById("m3uInput").addEventListener("change", function(e){
    archivoM3U = e.target.files[0];

    if (!archivoM3U) return;

    leerArchivoInicial();

    document.getElementById("autoStatus").innerHTML =
        "Monitoreando cada 5 segundos...";

    if (intervalo) clearInterval(intervalo);
    intervalo = setInterval(procesarDesdeMemoria, 5000);
});

function leerArchivoInicial() {
    const lector = new FileReader();
    lector.onload = function(e) {
        contenidoM3U = e.target.result;
        procesarDesdeMemoria();
    };
    lector.readAsText(archivoM3U);
}

function procesarDesdeMemoria() {

    if (!contenidoM3U || contenidoM3U.length < 5) return;

    const lineas = contenidoM3U.split(/\r?\n/);

    let ultima = null;

    for (let i = lineas.length - 1; i >= 0; i--) {
        if (/\.(mp3|mp4|wav|avi)$/i.test(lineas[i])) {
            ultima = lineas[i];
            break;
        }
    }
    ...
    const nombre = limpiarNombre(ultima);

    // 🚫 EVITAR BUCLE: si ya la procesamos, ignorar
    if (nombre === ultimaCancionProcesada) {
        console.log("Canción repetida, ignorando:", nombre);
        return;
    }

    // ✔️ Guardamos para no repetir
    ultimaCancionProcesada = nombre;

    console.log("Nueva canción detectada:", nombre);

    // ENVIAR A PHP
    fetch("auto_match.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
    ...
    .then(data => {
        if (data.marcadas > 0) {
            document.getElementById("autoStatus").innerHTML =
                `Marcadas como Sonada: ${data.marcadas}`;
        }
    });
}

function limpiarNombre(ruta) {
    let base = ruta.split(/\\|\//).pop();
    return base.replace(/\.(mp3|mp4|wav|avi)$/i, "").trim();
}
